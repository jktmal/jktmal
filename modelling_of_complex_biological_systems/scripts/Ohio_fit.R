...
fit <- stan(file = 'scripts/Ohio_final.stan', init = init_fn, 
            data = OH_dat, chains = 4, iter = 10000,
            control = list(max_treedepth = 10, adapt_delta = 0.995))
saveRDS(fit, "results/OH_fit.rds")

sims <- rstan::extract(fit)
save(sims,file = "results/OH_sims.RData")
