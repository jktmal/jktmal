...
def spin_mc(nbmatrix, spin, kt):
	energy = np.zeros( 10000, dtype='float')
	energy[0] = 
...
S = np.random.random(size=args.n)
S[S<=0.5] = -1
S[S>0.5] = 1
...
lam = 1
kt = 0.1
kt = np.linspace(0.1, 1.5, 50) # wcześniej 0.01, 10, 100
energy = np.zeros( (len(kt), kroki+1), dtype='float')

#A.resize(1,args.n*args.n)
E_pocz = energia(create_nbmatrix(A, args.m))
energy[0][0] = E_pocz
...
for i in range(len(kt)):
	...
	if i%args.m==0 and i!=0 and np.abs(A[args.n//2]-args.m*(args.m//2+1/2)) >= (args.m//4):
		A = A - args.m//4
		print('check')	
	lock = 0
	for j in range(kroki):
		...
		#A[0][h] = A[0][h]*(-1)
		h()
		E_konc = energia(create_nbmatrix(A, args.m))
		#A.resize(1,args.n*args.n)
		if E_konc < E_pocz:
			E_pocz = E_konc
			lock = 0
